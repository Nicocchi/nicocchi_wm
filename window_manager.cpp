...
void WindowManager::Run() {
    ...
    //  c. Grab X server to prevent window from changing under us while we frame them.
    ...
    CHECK(XQueryTree(display_, root_, &returned_root, &returned_parent, &top_level_windows,
...
void WindowManager::Frame(Window w, bool was_created_before_window_manager) {
    ...
    // We shouldn't be framing window we've already framed.
    CHECK(!clients_.count(w));

    // 1. Retrieve attributes fo window to frame.
    ...
    // 3. Create frame.
    const Window frame = XCreateSimpleWindow(display_, root_, x_window_attrs.x, x_window_attrs.y,
                                             x_window_attrs.width, x_window_attrs.height,
                                             BORDER_WIDTH, BORDER_COLOR, BG_COLOR);

    // 4. Select evetns on frame
    ...
    // 8. Save frame handle
    clients_[w] = frame;
    // 9. Grab events for window management actions on client window.
    //  a. Move windows with alt + left button
    XGrabButton(display_, Button1, Mod1Mask, w, false,
                ButtonPressMask | ButtonReleaseMask | ButtonMotionMask, GrabModeAsync,
                GrabModeAsync, None, None);
    //  b. Resize windows with alt + right button
    XGrabButton(display_, Button3, Mod1Mask, w, false,
                ButtonPressMask | ButtonReleaseMask | ButtonMotionMask, GrabModeAsync,
                GrabModeAsync, None, None);
    //  c. Kill windows with alt + f4 button
    XGrabKey(display_, XKeysymToKeycode(display_, XK_F4), Mod1Mask, w, false, GrabModeAsync,
             GrabModeAsync);
    //  d. Switch windows with alt + tab button
    XGrabKey(display_, XKeysymToKeycode(display_, XK_Tab), Mod1Mask, w, false, GrabModeAsync,
             GrabModeAsync);
...
void WindowManager::OnUnmapNotify(const XUnmapEvent &e) {
    ...
    // need to check because we will receive an UnmapNotify event for a frame
    ...
    // unmapped. This means taht an UnmapNotify event from a normal client window
...
void WindowManager::OnButtonPress(const XButtonEvent &e) {
    ...
    CHECK(XGetGeometry(display_, frame, &returned_root, &x, &y, &width, &height, &border_width,
                       &depth));
...
void WindowManager::OnMotionNotify(const XMotionEvent &e) {
    ...
    if (e.state & Button1Mask) {
        ...
        const Vector2D<int> size_delta(max(delta.x, -drag_start_frame_size_.width),
